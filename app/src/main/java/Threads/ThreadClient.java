...
public class ThreadClient extends Thread {
    ...
    public ThreadClient(LoginActivity loginActivity)
    {
        mainAct = loginActivity;
    }
    public void run()
    {
        int i = 0;
        while(finish) {
            ...
            if (finish) {
                mainAct.runOnUiThread(new Runnable() {
                    public void run() {
                        Toast.makeText(mainAct, "Connexion avec le serveur Athala établie ...", Toast.LENGTH_SHORT).show();
                        mainAct.ConnectButton.setEnabled(true);
                    }
                });
                Socket CSocket = null;
